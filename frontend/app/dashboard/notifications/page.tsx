...
const resolveActionUrl = (row: NotificationItem): string | undefined => {
  ...
  const workflowStep = Number(row.meta?.workflow_step || 0);
  const loanRequestId = Number(row.meta?.loan_request_id || row.meta?.loan_id || 0);
  const customerNo = String(row.meta?.customer_no || '').trim();
  const referenceNo = String(row.meta?.reference_no || '').trim();
  const loanCode = String(row.meta?.loan_code || '').trim();

  if (workflowStep === 14 || row.type === 'step_14') {
    const params = new URLSearchParams();
    if (loanRequestId > 0) params.set('loan_id', String(loanRequestId));
    if (customerNo) params.set('customer_no', customerNo);
    if (referenceNo) params.set('reference_no', referenceNo);
    if (loanCode) params.set('loan_code', loanCode);

    const query = params.toString();
    return query
      ? `/dashboard/microfinance/loans/released?${query}`
      : '/dashboard/microfinance/loans/released';
  }
